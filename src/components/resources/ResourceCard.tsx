...
export function ResourceCard({ resource }: { resource: DeveloperResource }) {
  return (
    <div className="group relative flex flex-col gap-3 rounded-xl border border-ink-100 p-5 transition hover:border-brand-200 hover:shadow-sm">
      <a
        href={resource.url}
        target="_blank"
      ...
      <div className="pointer-events-none relative z-[1] flex items-start justify-between gap-2">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-ink-50">
          {resource.iconUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={resource.iconUrl} alt="" className="h-6 w-6 object-contain" />
          ) : (
            <span className="text-sm font-bold text-ink-400">{resource.title[0]}</span>
          )}
        </div>
        {resource.isFeatured && (
          <span className="flex items-center gap-1 rounded-full bg-amber-50 px-2 py-0.5 text-[11px] font-semibold text-amber-700 ring-1 ring-inset ring-amber-100">
      ...
      <div className="pointer-events-none relative z-[1]">
        <h3 className="flex items-center gap-1.5 font-semibold text-ink-900 transition group-hover:text-brand-600">
          {resource.title}
          <ExternalLink size={13} className="shrink-0 text-ink-300 transition group-hover:text-brand-500" />
        </h3>
        ...
        {resource.tags.length > 0 ? (
          <div className="pointer-events-none flex flex-wrap gap-1.5">
            {resource.tags.slice(0, 4).map((tag) => (
              <span key={tag} className="rounded-full bg-ink-50 px-2 py-0.5 text-[11px] font-medium text-ink-500">
                #{tag}
              </span>
            ))}
          </div>
