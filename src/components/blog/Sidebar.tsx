...
export async function Sidebar() {
  ...
  return (
    <aside className="flex flex-col gap-6">
            ...
            <li key={post.id}>
              <Link href={`/blog/${post.slug}`} className="group flex gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-[11px] font-medium text-ink-400">{readingTimeLabel(post.readingTimeMins)}</p>
                </div>
              </Link>
            </li>
          ))}
      ...
      <div className="rounded-2xl border border-slate-200/80 bg-white/80 p-5 shadow-sm backdrop-blur-md">
        <h3 className="mb-4 text-xs font-bold uppercase tracking-wider text-brand-600">Browse by tag</h3>
        <div className="flex flex-wrap gap-2">
          {safeTags.slice(0, 16).map((tag) => (
            <Link key={tag.id} href={`/tag/${tag.slug}`}>
              <Badge variant="outline" className="px-2.5 py-1 text-xs font-medium transition hover:border-brand-300 hover:bg-brand-50/50 hover:text-brand-700">
                #{tag.name}
              </Badge>
            </Link>
          ))}
