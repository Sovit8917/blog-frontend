  ...
  return (
    <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <div className="col-span-2 flex flex-col gap-6">
        <Link
          href={`/blog/${main.slug}`}
          className="group relative block overflow-hidden rounded-2xl border border-slate-200/80 dark:border-slate-700 bg-white dark:bg-ink-900 p-5 sm:p-6 shadow-xs transition hover:border-slate-300 dark:hover:border-slate-600 hover:shadow-md"
        >
          {main.coverImageUrl && (
          ...
          <div className="space-y-2.5">
            ...
            <h2 className="text-xl font-bold tracking-tight text-ink-950 dark:text-ink-50 sm:text-2xl group-hover:text-brand-600 dark:group-hover:text-brand-400 transition">
              {main.title}
            </h2>
        ...
        {jobs.length > 0 && (
          <div className="rounded-2xl border border-slate-200/70 dark:border-slate-700 bg-white dark:bg-ink-900 p-6 shadow-xs">
            <div className="mb-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
              <div className="flex items-center gap-2">
                <span className="flex h-6 w-6 items-center justify-center rounded-md bg-brand-50 dark:bg-brand-900/40 text-brand-600 dark:text-brand-400">
                  <Briefcase size={14} />
                </span>
