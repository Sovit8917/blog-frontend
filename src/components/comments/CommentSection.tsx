...
export function CommentSection({ postId, initialComments }: { postId: string; initialComments: Comment[] }) {
  const [comments] = useState<Comment[]>(Array.isArray(initialComments) ? initialComments : []);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setDraft('');
      setNotice('Thanks — your comment is awaiting moderation and will appear shortly.');
    } catch {
  ...
  return (
    <section id="comments" className="mt-16 border-t border-ink-100 pt-10">
      <h2 className="mb-6 flex items-center gap-2 text-xl font-bold text-ink-900">
        <MessageCircle size={20} /> Comments ({Array.isArray(comments) ? comments.length : 0})
      </h2>
      ...
      <ul className="space-y-6">
        {Array.isArray(comments) && comments.map((c) => (
          <li key={c.id} className="flex gap-3">
            <Avatar src={c.user?.avatarUrl} name={c.user?.name ?? 'Anonymous'} size={36} />
            <div className="flex-1">
              <div className="flex items-baseline gap-2">
                <span className="text-sm font-semibold text-ink-900">{c.user?.name ?? 'Anonymous'}</span>
                <span className="text-xs text-ink-400">{timeAgo(c.createdAt)}</span>
              </div>
              <p className="mt-1 text-sm leading-relaxed text-ink-700">{c.content}</p>
            </div>
          </li>
        ))}
