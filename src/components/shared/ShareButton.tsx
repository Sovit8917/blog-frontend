...
export function ShareButton({
      ...
      >
        ...
        {isOpen && (
          <div
            ...
            className="absolute bottom-full right-0 mb-2 z-50 flex w-48 flex-col rounded-2xl border border-slate-200/90 dark:border-slate-700 bg-white dark:bg-ink-900 p-2 shadow-xl ring-1 ring-black/5 animate-in fade-in zoom-in-95 duration-150"
          >
            ...
            {links.map(({ channel, label, href, svgPath, iconColor }) => (
              <a
                ...
                className="flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-xs font-semibold text-slate-700 dark:text-slate-300 transition hover:bg-slate-50 dark:hover:bg-slate-800"
              >
                <svg
  ...
  return (
    <div className={cn("flex items-center gap-2 py-0.5", className)}>
      ...
      {links.map(({ channel, label, href, svgPath, btnStyle }) => (
        <a
          key={channel}
          href={href}
          target="_blank"
