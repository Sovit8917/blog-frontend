  ...
  return (
    <div className={`prose prose-ink max-w-none ${className}`}>
      <ReactMarkdown
        ...
        rehypePlugins={
          slugHeadings
            ? [rehypeSlug, [rehypeAutolinkHeadings, { behavior: 'append', properties: { className: ['anchor-link'], ariaHidden: true, tabIndex: -1 } }]]
            : []
        }
        components={{
          img: ({ src, alt }) =>
          ...
          tr: ({ children }) => <tr className="border-b border-ink-100 last:border-0">{children}</tr>,
          th: ({ children }) => (
          ...
          code: ({ children, className: codeClassName }) =>
            codeClassName ? (
              ...
              <code className="rounded bg-ink-100 px-1.5 py-0.5 text-[0.9em] text-brand-700">{children}</code>
            ),
        }}
      >
        {content}
