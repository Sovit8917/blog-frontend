...
export function RelatedJobsStrip({ jobs }: { jobs: JobCardType[] }) {
  ...
  return (
    <section className="rounded-2xl border border-ink-100 bg-ink-50/50 p-5 sm:p-6">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-sm font-bold uppercase tracking-wide text-ink-500">Jobs from the community</h2>
        <Link href="/jobs" className="flex items-center gap-1 text-xs font-semibold text-brand-600 hover:text-brand-700">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {jobs.slice(0, 3).map((job) => (
          <Link
            key={job.id}
            href={`/jobs/${job.slug}`}
            className="group flex flex-col gap-2 rounded-xl border border-ink-100 bg-white p-4 transition hover:border-brand-200 hover:shadow-sm"
          >
            <div className="flex items-center gap-2">
              <div className="relative flex h-7 w-7 shrink-0 items-center justify-center overflow-hidden rounded-md bg-ink-100">
                {job.company.logoUrl ? (
                  ...
                  <span className="text-[10px] font-bold text-ink-400">{job.company.name[0]}</span>
                )}
              </div>
              <span className="truncate text-xs font-medium text-ink-500">{job.company.name}</span>
            </div>
            ...
            <div className="mt-auto flex items-center justify-between text-[11px] text-ink-400">
              <span className="flex items-center gap-1">
                <MapPin size={11} /> {job.location ?? REMOTE_TYPE_LABEL[job.remoteType]}
              </span>
