  ...
  return (
    <section className="rounded-2xl border border-ink-100 bg-white p-5 sm:p-6">
      <div className="mb-4 flex items-center gap-2">
        ...
        <h2 className="text-sm font-bold uppercase tracking-wide text-ink-700">Continue reading</h2>
      </div>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {inProgress.map((entry) => (
          <Link
            ...
            className="group flex gap-3 rounded-xl border border-ink-100 p-3 transition hover:border-brand-200 hover:shadow-sm"
          >
            <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-lg bg-ink-100">
              {entry.post.coverImageUrl ? (
                ...
                <span className="flex h-full w-full items-center justify-center text-sm font-bold text-ink-400">
                  {entry.post.title[0]}
                </span>
              )}
            </div>
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-[11px] font-medium text-ink-400">{entry.progressPct}% done</p>
            </div>
          </Link>
        ))}
      </div>
