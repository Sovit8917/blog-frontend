...
export async function NewsletterSponsorLine() {
  ...
  return (
    <div className="mx-auto mt-4 flex max-w-sm items-center justify-center gap-2 rounded-lg bg-white/10 px-3 py-2 text-xs text-ink-200">
      <span className="font-semibold text-white">Sponsored by {slot.sponsor.name}:</span>
      <NewsletterSponsorClickLink
        slotId={slot.id}
        url={slot.url}
        className="underline decoration-ink-400 underline-offset-2 hover:text-white"
      >
        {slot.headline}
      </NewsletterSponsorClickLink>
    </div>
