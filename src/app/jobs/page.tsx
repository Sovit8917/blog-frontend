...
export default async function JobsPage({ searchParams }: Props) {
  ...
  const params: ListJobsParams = {
    ...
    freshersOnly: String(searchParams.freshersOnly) === "true" || undefined,
    verifiedOnly: String(searchParams.verifiedOnly) === "true" || undefined,
  ...
  return (
    <div className="bg-slate-50/50 py-8 lg:py-12">
      <div className="container-page space-y-8">
        ...
        {currentPage === 1 && !params.cursor && !hasActiveFilters && (
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_320px]">
          <div className="rounded-2xl border border-slate-200/70 bg-white p-6 sm:p-8 shadow-sm space-y-6">
            ...
            {page.items.length === 0 ? (
              ...
              <div>
                {/* BETWEEN_POSTS sits inline, midway through the list — not
                    after it — so it actually gets scrolled past while people
                    are still reading rows, not skipped once they've already
                    hit the pagination controls. */}
                {page.items
                  .slice(0, Math.ceil(page.items.length / 2))
                  .map((job) => (
                    <JobListRow key={job.id} job={job} />
                  ))}
                {page.items.length > 1 && (
                  <AdSlot placement="BETWEEN_POSTS" className="my-6" />
                )}
                {page.items
                  .slice(Math.ceil(page.items.length / 2))
                  .map((job) => (
                    <JobListRow key={job.id} job={job} />
                  ))}
              </div>
