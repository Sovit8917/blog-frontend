  ...
  return (
    <div className="container-page py-10">
      <header className="mb-8">
        <h1 className="text-3xl font-bold text-ink-900 dark:text-ink-100">Reading History</h1>
        <p className="mt-2 text-ink-500 dark:text-ink-400">Articles you&apos;ve read, most recent first.</p>
      </header>
      ...
      {history.length === 0 ? (
        ...
        <ul className="flex flex-col gap-4">
          {history.map((h) => (
            <li key={h.id} className="flex items-center gap-4 rounded-xl border border-ink-100 dark:border-ink-800 p-4">
              <div className="relative flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-ink-100 dark:bg-ink-800">
                {h.post.coverImageUrl ? (
                  ...
                  <span className="text-sm font-bold text-ink-400 dark:text-ink-500">{h.post.title[0]}</span>
                )}
              </div>
              <div className="flex-1">
                <Link href={`/blog/${h.post.slug}`} className="font-semibold text-ink-900 dark:text-ink-100 hover:text-brand-600 dark:hover:text-brand-400">
                  {h.post.title}
                </Link>
                <p className="text-sm text-ink-500 dark:text-ink-400">{h.post.author?.name}</p>
                <p className="mt-0.5 text-xs text-ink-400 dark:text-ink-500">
                  Read {timeAgo(h.readAt)} · {h.progressPct}% through
                </p>
              </div>
              <div className="h-1.5 w-16 shrink-0 overflow-hidden rounded-full bg-ink-100 dark:bg-ink-800">
                <div
