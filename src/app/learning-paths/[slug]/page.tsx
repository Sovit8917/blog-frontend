    ...
    <div className="bg-slate-50/50 py-8 lg:py-12">
      <div className="container-page max-w-3xl space-y-8">
        <header className="rounded-2xl border border-slate-200/70 bg-white p-6 sm:p-8 shadow-sm">
          ...
          <h1 className="mt-1 font-serif text-3xl font-extrabold text-ink-950 sm:text-4xl">{path.title}</h1>
          {path.description && (
            <p className="mt-2 max-w-2xl text-base leading-relaxed text-ink-600">{path.description}</p>
          )}
        </header>

        <ol className="space-y-4">
          {path.steps.map((step, i) => (
            <li key={step.id} className="flex gap-4 rounded-xl border border-ink-100 bg-white p-5">
              ...
              <div className="min-w-0 flex-1">
                <a
                  ...
                  className="flex items-center gap-1.5 font-serif text-lg font-bold text-ink-950 hover:text-brand-600"
                >
                  {step.resource.title} <ExternalLink size={14} className="shrink-0 text-ink-400" />
                </a>
                {step.note && <p className="mt-1 text-sm font-medium text-brand-700">{step.note}</p>}
                {step.resource.description && (
                  <p className="mt-1.5 text-sm leading-relaxed text-ink-600">{step.resource.description}</p>
                )}
              </div>
            </li>
          ))}
        ...
        <div className="text-center">
          <Link href="/learning-paths" className="link-underline text-ink-700">
